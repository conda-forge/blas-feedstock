...
if [[ "${blas_impl}" == "blis" ]]; then
  ...
  conda install -c conda-forge/label/lapack_rc -c conda-forge "libblas=*=*blis" "libcblas=*=*blis" "liblapack=3.11=2*netlib" "liblapacke=3.11=2*netlib" "blis=0.9.0=*_2" --use-local --yes -p $PREFIX
fi
